...
def prepare_add_to_db(results, id, source, forced, title, channel, thumbnail):
    with app.app_context():
        try:
            model_service = ModelService()
            video_subtitles_details = results[0]
            video_keywords_img = results[1]
            model_service.create_video_lesson(id, video_subtitles_details, video_keywords_img, source, forced, title, channel, thumbnail)
...
def execute_transcript_tasks(id, transcript, source, forced, title, channel, thumbnail):
    ...
    results = chord(prepare_lesson)(prepare_add_to_db.s(id, source, forced, title, channel, thumbnail))
...
def execute_transcript_tasks_non_youtube(id, transcript, source, forced, title, channel, thumbnail):
    youtube_helper = YouTubeHelper()
