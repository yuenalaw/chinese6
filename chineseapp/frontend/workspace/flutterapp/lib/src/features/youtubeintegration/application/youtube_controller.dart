...
class YouTubeController extends StateNotifier<AsyncValue<QueriedVideo>> {
  YouTubeController({required this.youTubeService}) : super(const AsyncValue.loading());

  Future<void> searchVideo(String vidId) async {
    state = const AsyncLoading();
    state = await AsyncValue.guard( 
      () => youTubeService.searchVideo(vidId)
    );
  }
...
final youtubeControllerProvider = StateNotifierProvider<YouTubeController, AsyncValue<QueriedVideo>>((ref) {
