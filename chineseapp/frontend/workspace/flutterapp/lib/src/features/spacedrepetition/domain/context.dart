/*
{
    "context": [
        {
            "duration": 1.291,
            "segment": "加州留学生的生活",
            "sentences": {
                "entries": [
                    {
                        "pinyin": "jia zhou",
                        "similarsounds": [
                            "甲胄",
                            "甲冑"
                        ],
                        "translation": [
                            [
                                "加",
                                [
                                    "to add",
                                    "plus",
                                    "(used after an adverb such as 不, 大, 稍 etc, and before a disyllabic verb, to indicate that the action of the verb is applied to sth or sb previously mentioned)",
                                    "to apply (restrictions etc) to (sb)",
                                    "to give (support, consideration etc) to (sth)"
                                ]
                            ],
                            [
                                "加州",
                                [
                                    "California"
                                ]
                            ],
                            [
                                "州",
                                [
                                    "prefecture",
                                    "(old) province",
                                    "(old) administrative division",
                                    "state (e.g. of US)",
                                    "oblast (Russia)",
                                    "canton (Switzerland)"
                                ]
                            ]
                        ],
                        "upos": "PROPN",
                        "word": "加州"
                    },
                    {
                        "pinyin": "liu xue",
                        "similarsounds": [
                            "流血",
                            "留学"
                        ],
                        "translation": [
                            [
                                "留",
                                [
                                    "old variant of 留[liu2]"
                                ]
                            ],
                            [
                                "留学",
                                [
                                    "to study abroad"
                                ]
                            ],
                            [
                                "学",
                                [
                                    "to learn",
                                    "to study",
                                    "to imitate",
                                    "science",
                                    "-ology"
                                ]
                            ]
                        ],
                        "upos": "VERB",
                        "word": "留学"
                    },
                    {
                        "pinyin": "sheng",
                        "similarsounds": [
                            "籸",
                            "审",
                            "兟",
                            "什",
                            "省"
                        ],
                        "translation": [
                            [
                                "生",
                                [
                                    "to be born",
                                    "to give birth",
                                    "life",
                                    "to grow",
                                    "raw",
                                    "uncooked",
                                    "student"
                                ]
                            ]
                        ],
                        "upos": "PART",
                        "word": "生"
                    },
                    {
                        "pinyin": "de",
                        "similarsounds": [
                            "徳",
                            "㝵",
                            "淂",
                            "德",
                            "㥁"
                        ],
                        "translation": [
                            [
                                "的",
                                [
                                    "aim",
                                    "clear"
                                ]
                            ]
                        ],
                        "upos": "PART",
                        "word": "的"
                    },
                    {
                        "pinyin": "sheng huo",
                        "similarsounds": [
                            "甚或",
                            "生火",
                            "生活",
                            "身后",
                            "深厚"
                        ],
                        "translation": [
                            [
                                "生",
                                [
                                    "to be born",
                                    "to give birth",
                                    "life",
                                    "to grow",
                                    "raw",
                                    "uncooked",
                                    "student"
                                ]
                            ],
                            [
                                "生活",
                                [
                                    "life",
                                    "activity",
                                    "to live",
                                    "livelihood"
                                ]
                            ],
                            [
                                "活",
                                [
                                    "to live",
                                    "alive",
                                    "living",
                                    "work",
                                    "workmanship"
                                ]
                            ]
                        ],
                        "upos": "NOUN",
                        "word": "生活"
                    }
                ],
                "sentence": "加州留学生的生活"
            },
            "start": 10.708
        },
        {
            "duration": 1.958,
            "segment": "晚上在夜店喝酒狂欢",
            "sentences": {
                "entries": [
                    {
                        "pinyin": "wan shang",
                        "similarsounds": null,
                        "translation": [
                            [
                                "晚",
                                [
                                    "evening",
                                    "night",
                                    "late"
                                ]
                            ],
                            [
                                "晚上",
                                [
                                    "evening",
                                    "night",
                                    "CL:個|个[ge4]",
                                    "in the evening"
                                ]
                            ],
                            [
                                "上",
                                [
                                    "on top",
                                    "upon",
                                    "above",
                                    "upper",
                                    "previous",
                                    "first (of multiple parts)",
                                    "to climb",
                                    "to get onto",
                                    "to go up",
                                    "to attend (class or university)"
                                ]
                            ]
                        ],
                        "upos": "NOUN",
                        "word": "晚上"
                    },
                    {
                        "pinyin": "zai",
                        "similarsounds": [
                            "縡",
                            "咂",
                            "扎",
                            "灾",
                            "仔"
                        ],
                        "translation": [
                            [
                                "在",
                                [
                                    "(located) at",
                                    "(to be) in",
                                    "to exist",
                                    "in the middle of doing sth",
                                    "(indicating an action in progress)"
                                ]
                            ]
                        ],
                        "upos": "VERB",
                        "word": "在"
                    },
                    {
                        "pinyin": "ye",
                        "similarsounds": null,
                        "translation": [
                            [
                                "夜",
                                [
                                    "night"
                                ]
                            ]
                        ],
                        "upos": "NOUN",
                        "word": "夜"
                    },
                    {
                        "pinyin": "dian",
                        "similarsounds": [
                            "琠",
                            "癫",
                            "碘",
                            "扂",
                            "巓"
                        ],
                        "translation": [
                            [
                                "店",
                                [
                                    "inn",
                                    "shop",
                                    "store",
                                    "CL:家[jia1]"
                                ]
                            ]
                        ],
                        "upos": "PART",
                        "word": "店"
                    },
                    {
                        "pinyin": "he",
                        "similarsounds": [
                            "阂",
                            "嵙",
                            "涸",
                            "坷",
                            "蠚"
                        ],
                        "translation": [
                            [
                                "喝",
                                [
                                    "to shout loudly"
                                ]
                            ]
                        ],
                        "upos": "VERB",
                        "word": "喝"
                    },
                    {
                        "pinyin": "jiu",
                        "similarsounds": [
                            "糺",
                            "就",
                            "丩",
                            "赳",
                            "揫"
                        ],
                        "translation": [
                            [
                                "酒",
                                [
                                    "wine (esp. rice wine)",
                                    "liquor",
                                    "spirits",
                                    "alcoholic beverage",
                                    "CL:杯[bei1],瓶[ping2],罐[guan4],桶[tong3],缸[gang1]"
                                ]
                            ]
                        ],
                        "upos": "NOUN",
                        "word": "酒"
                    },
                    {
                        "pinyin": "kuang",
                        "similarsounds": [
                            "匡",
                            "軖",
                            "光",
                            "广",
                            "丱"
                        ],
                        "translation": [
                            [
                                "狂",
                                [
                                    "mad",
                                    "wild",
                                    "violent"
                                ]
                            ]
                        ],
                        "upos": "VERB",
                        "word": "狂"
                    },
                    {
                        "pinyin": "huan",
                        "similarsounds": [
                            "桓",
                            "髋",
                            "鬟",
                            "洹",
                            "缓"
                        ],
                        "translation": [
                            [
                                "欢",
                                [
                                    "a breed of horse",
                                    "variant of 歡|欢[huan1]"
                                ]
                            ]
                        ],
                        "upos": "NOUN",
                        "word": "欢"
                    }
                ],
                "sentence": "晚上在夜店喝酒狂欢"
            },
            "start": 13.708
        }
    ],
    "message": "Successfully obtained context!"
}
*/

class Translation {
  final String word;
  final List<String> translations;

  Translation({required this.word, required this.translations});

  factory Translation.fromJson(List<dynamic> json) {
    return Translation(
      word: json[0] as String,
      translations: List<String>.from(json[1].map((x) => x as String)),
    );
  }
}

class Entry {
  final String pinyin;
  final List<String>? similarsounds;
  final List<Translation> translation;
  final String upos;
  final String word;

  Entry({
    required this.pinyin,
    this.similarsounds,
    required this.translation,
    required this.upos,
    required this.word,
  });

  factory Entry.fromJson(Map<String, dynamic> json) {
    return Entry(
      pinyin: json['pinyin'] as String,
      similarsounds: json['similarsounds']?.map<String>((e) => e as String).toList(),
      translation: json['translation'] != null ? (json['translation'] as List).map((e) => Translation.fromJson(e as List)).toList() : [],
      upos: json['upos'] as String,
      word: json['word'] as String,
    );
  }
}

class Sentence {
  final String sentence;
  final List<Entry> entries;

  Sentence({
    required this.sentence,
    required this.entries,
  });

  factory Sentence.fromJson(Map<String, dynamic> json) {
    return Sentence(
      sentence: json['sentences']['sentence'] as String,
      entries: (json['sentences']['entries'] as List).map((e) => Entry.fromJson(e as Map<String, dynamic>)).toList(),
    );
  }
}

class Context {
  final Sentence previousSentence;
  final Sentence nextSentence;

  Context({
    required this.previousSentence,
    required this.nextSentence,
  });

  factory Context.fromJson(Map<String, dynamic> json) {
    return Context(
      previousSentence: Sentence.fromJson(json['context'][0]),
      nextSentence: Sentence.fromJson(json['context'][1]),
    );
  }
}